...
### B2048 决定性实验（2026-08-26，用户提供）：H9 排除
MATH500 B2048 + chat 统一协议（完全对齐训练分布）：**Base − E2 = 0.116 ≥ 0.05 ->
H9（预算错位/截断假象）排除**；E2 no_answer 仅 1.2%--模型完成推理但错得多，
损害是真实的。旧 B512 差距（-0.108）与 B2048（-0.116）同向且量级一致，进一步
确认与预算无关。
