...
**损害的形态**：集中在**长预算能力上限**（B2048 -0.116/-0.138）；短预算（B512 chat）
学生反超 Base（+0.028/+0.032）；avg_rt 三模型几乎一致（1840-1954）--非行为模式剧变
（长度/循环），是**推理质量本身下降**。

-> **归因分析与修复方案见 `docs/reports/2026-08-26-opd-failure-analysis.md`**
（根因树 RC1 固定D偏离论文on-policy / RC2 代理门控 / RC3 弱KL+500样本重放 /
RC4 Δ_T语义未验证；判别实验 E-0 拐点扫描 + E-1 Δ↔correctness 相关性；
KL 档位扫描建议在 E-1 判定信号生死后再启动）。
