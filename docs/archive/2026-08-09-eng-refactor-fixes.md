...
> **状态：已完成（2026-08-18 归档）。** A1-A7/B1-B5/C1-C5/D1-D5 全部实现并通过测试（对应设计见 docs/specs/2026-08-09-eng-refactor-fixes-design.md）。


> **状态（2026-08-16 同步）：** 工程化二次审查修复（A1-A7/B1-B5/C1-C5/D1-D5）已全部实现并通过测试；新增 L2/G5/G7/P1 项亦已落地。
...
- [x] **步骤 1：写失败测试**（checkpoint：文件路径构造不崩 + load 正常；cli eval 端到端）
...
- [x] **步骤 2：运行确认失败**——`pytest tests/test_cli.py::test_cli_eval_end_to_end -q` 应 FileExistsError
- [x] **步骤 3：实现**：`_cmd_eval` 改为
```python
ck = CheckpointManager(os.path.dirname(args.checkpoint)).load(args.checkpoint)
```
（不再把文件路径当 checkpoint_dir）
- [x] **步骤 4：`pytest tests/test_checkpoint.py tests/test_cli.py -q` 全过；`pytest tests/ -q` 无回归**
- [x] **步骤 5：Commit** `fix(cli): eval 子命令 FileExistsError + eval/cache 端到端测试（A2/D1）`
...
```
- [x] **步骤 2：确认失败**（当前 load_config 不下渗 → stage1 无 cache_mode）
- [x] **步骤 3：实现**：config.py 加 `_seep_deployment_keys(d)`，在 load_config 里、pydantic 校验**前**对 data 做下渗：
```python
...
- [x] **步骤 4：删除 pipeline.py 里 s1cfg/s2cfg 的下渗循环**（直接 `self.cfg["stage1"]` / `self.cfg["stage2"]`）；`_cmd_cache` 同样删循环。全量测试过。
- [x] **步骤 5：Commit** `refactor(config): 部署键下渗移到 load_config，快照即有效配置（A4/A5/B4）`
...
```
- [x] **步骤 2：确认失败**（当前 n_step_off 通过校验）
- [x] **步骤 3：实现**：`Stage2Cfg.scheduling_mode: Literal["fully_async"] = "fully_async"`；`DEFAULT_CONFIG_V2["stage2"]["scheduling_mode"]="fully_async"`；`configs/fullstack_opd.yaml` 已是 fully_async 无需改
- [x] **步骤 4：`pytest tests/ -q` 全过**
- [x] **步骤 5：Commit** `fix(config): scheduling_mode 收窄为已实现的 fully_async（A6）`
...
- [x] **步骤 1：写失败测试**（模拟分布式分支不引用未定义 scheduler）
```python
...
- [x] **步骤 4：`pytest tests/test_pipeline.py -q` 全过；全量无回归**
- [x] **步骤 5：Commit** `fix(pipeline): distributed 分支不引用未定义 scheduler（A1）`
...
```
- [x] **步骤 2：确认失败**（异常时 close_logging 未执行 → FileHandler 残留）
- [x] **步骤 3：实现**：run() 主体包 `try: ... finally: mr.close(); close_logging("opd")`；成功路径的 close 移到 finally
- [x] **步骤 4：全量测试过**
- [x] **步骤 5：Commit** `fix(pipeline): run() try/finally 保证异常也释放资源（A7）`
...
```
- [x] **步骤 2：确认失败**（当前裸 ValidationError / ValueError）
- [x] **步骤 3：实现**：`load_config` 里包 `try: OPDConfig(**data) except ValidationError as e: raise ConfigError(...) from e`；`--set` 缺 `=` 抛 `ConfigError`；warmup 校验 `raise DataError`（改 pipeline.py:188）；CLI `main` 捕 `(OPDError, ValidationError)` 统一友好退出（返回值 2）
- [x] **步骤 4：全量测试过**
- [x] **步骤 5：Commit** `refactor(exceptions): 配置/数据错误收敛 OPDError 子类（B1）`
...
def test_build_model_used_for_student(monkeypatch):
    ...
    assert calls
```
- [x] **步骤 2：确认失败**（build_model 未被调用）
- [x] **步骤 3：实现**：pipeline 的 `student = build_model(self.cfg, self.device, role="student")`；`_stage0_teachers` 内教师构造也走 `build_model(self.cfg, self.device, role="teacher")`
- [x] **步骤 4：全量测试过**（toy 行为不变）
- [x] **步骤 5：Commit** `feat(pipeline): build_model 接入 student/teacher 构造（B2）`
...
```
- [x] **步骤 2：确认失败**（当前无参可能报错或不同）
- [x] **步骤 3：实现**：删 `demo.py`；`run_fullstack_v2.py` 改 `from fullstack_opd_v2.cli import main`；README 与 CLAUDE.md 的 `python -m fullstack_opd_v2` 改为 `... train ...`；argparse `add_subparsers(required=True)` 已使无参报错
- [x] **步骤 4：全量测试过**
- [x] **步骤 5：Commit** `refactor(cli): 清理 demo.py 死代码，统一入口为子命令（B3）`
...
```
- [x] **步骤 2：确认失败**（csv_path 被忽略，写默认 metrics.csv）
- [x] **步骤 3：实现**：`MetricsRecorder(backend=..., run_dir=paths["run_dir"], csv_path=mcfg.get("csv_path"), wandb_project=...)`
- [x] **步骤 4：全量测试过**
- [x] **步骤 5：Commit** `fix(metrics): metrics.csv_path 配置键接入 pipeline（B5）`
...
```
- [x] **步骤 2：确认失败**（save 无 ref 参数）
- [x] **步骤 3：实现**：`CheckpointManager.save(..., ref=None)` 存 `ck["ref"]`；pipeline 在 `_on_step`/末步 save 传 `ref={"ref_dists":..., "ref_ids":..., "ref_logp":...}`（Stage 2 的 KL 锚点）；resume 时若 `ck.get("ref")` 有锚点则恢复、否则重算初始 student 锚点
- [x] **步骤 4：resume 强断言测试**
```python
...
```
- [x] **步骤 5：全量测试过**
- [x] **步骤 6：Commit** `feat(checkpoint): 断点存 ref 锚点，resume 恢复 KL 不变式（A3/D4）`
...
- [x] **步骤 1：写失败测试**（on_step 不阻塞——用短训练验证 metrics 仍落盘）
```python
...
```
- [x] **步骤 2：确认失败**（当前同步，仍通过——先实现再断言不崩）
- [x] **步骤 3：实现**：pipeline 起 daemon 消费线程 + `queue.Queue`；`_on_step` 只 `put((m, student_state))`；训练完 `join`；`MetricsRecorder.record` 加 `threading.Lock`
- [x] **步骤 4：全量测试过**
- [x] **步骤 5：Commit** `perf(pipeline): on_step checkpoint/metrics 异步后台落盘（C1）`
...
```
- [x] **步骤 2：确认失败**（无 flush_every 参数）
- [x] **步骤 3：实现**：`MetricsRecorder(..., flush_every=10)`；`record` 内 `if self._n_records % flush_every == 0: flush()`；`close` 终刷
- [x] **步骤 4：全量测试过**
- [x] **步骤 5：Commit** `perf(metrics): flush 节流 + close 终刷（C2）`
...
```
- [x] **步骤 2：确认通过（回归基线）**
- [x] **步骤 3：实现**：`_train_step` 里把 `loss.item()`/`pg_loss.item()`/`kl_loss.item()`/`adv.item()`/`reward.item()` 的 5 次设备同步收集为一次：先算标量张量，返回时统一 `[t.item() for t in ...]`
- [x] **步骤 4：全量测试过（数值不变）**
- [x] **步骤 5：Commit** `perf(scheduler): 热路径 5 次 .item() 设备同步收集为一次（C3）`
...
```
- [x] **步骤 2：确认失败**（每次 load 重建）
- [x] **步骤 3：实现**：`ToyDataLoader.load()` 用 `if self._cache is None: ... self._cache = (p, r, f)`；返回 `self._cache`
- [x] **步骤 4：全量测试过**
- [x] **步骤 5：Commit** `perf(data): ToyDataLoader 缓存避免重复重建（C4）`
...
```
- [x] **步骤 2：确认失败**（当前已 exist_ok，可能通过——验证 config 覆盖更新）
- [x] **步骤 3：实现**：`create()` 幂等（`exist_ok=True` 已有；确保 config.yaml 每次都重写为最新 cfg）
- [x] **步骤 4：全量测试过**
- [x] **步骤 5：Commit** `perf(run): RunManager.create 幂等（C5）`
...
```
- [x] **步骤 2：确认失败**（假绿/边界缺断言）
- [x] **步骤 3：实现**：若 metrics 缺失字段补齐行为确实有问题则修 metrics.py（`_ensure_fields` 已处理新字段）；其余纯测试补强
- [x] **步骤 4：全量测试过（94 + 新增全绿）**
- [x] **步骤 5：Commit** `test: 补强 metrics/checkpoint/pipeline 边界断言（D2/D3/D5）`
